...
void CubicProbing::createAccount(std::string id, int count) {
    ...
    if (float(hash_table_size)/float(capacity) >= 0.5){
        ...
        for (int z = 0; z < (capacity / 2); z++){
            if (bankStorage1d[z].id.size() == 22){
                ...
                j = 0;
                while (vec[(temp_hash_val + (j*j*j + j*j + j))%capacity].id.size() == 22){
                    j++;
                }
                vec[(temp_hash_val + (j*j*j + j*j + j))%capacity] = bankStorage1d[z];
            }
        }
        bankStorage1d = vec;
    }
    ...
    long long i = 0;
    while (bankStorage1d[(hash_val + (i*i*i + i*i + i))%capacity].id.size() == 22){
        i++;
    }
    bankStorage1d[(hash_val + (i*i*i + i*i + i))%capacity] = acc;
    hash_table_size++;
    return;
}

int CubicProbing::binary_search(std::vector<int> &res, int &val, int &cnt){
    int lo = 0;
    int hi = cnt - 1;
    int mid;
    if (cnt == 0){
